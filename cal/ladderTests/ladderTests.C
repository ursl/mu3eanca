...
void loadFiles(vector<string> &files, vector<int> &cols,
  ...
  if (0 == layer) {
    ...
    files.push_back("qc_ladder_1.json"); cols.push_back(kRed-9);      
    files.push_back("qc_ladder_2.json"); cols.push_back(kBlue-4);     
    files.push_back("qc_ladder_3.json"); cols.push_back(kMagenta+1);     
    files.push_back("qc_ladder_4.json"); cols.push_back(kYellow+3);     
    files.push_back("qc_ladder_5.json"); cols.push_back(kGreen-2);     
    files.push_back("qc_ladder_6.json"); cols.push_back(kViolet+1);     
    files.push_back("qc_ladder_7.json"); cols.push_back(kCyan+2);    
    
    files.push_back("qc_ladder_18.json"); cols.push_back(kRed+1);     
    files.push_back("qc_ladder_19.json"); cols.push_back(kRed-9);     
    files.push_back("qc_ladder_20.json"); cols.push_back(kBlue-4);    
    files.push_back("qc_ladder_21.json"); cols.push_back(kMagenta+1); 
    files.push_back("qc_ladder_22.json"); cols.push_back(kYellow+3);  
    files.push_back("qc_ladder_23.json"); cols.push_back(kGreen-2);   
    files.push_back("qc_ladder_24.json"); cols.push_back(kViolet+1);      
    files.push_back("qc_ladder_25.json"); cols.push_back(kCyan+2);    
  };                                     
  
  if (1 == layer) {
    files.push_back("qc_ladder_8.json");  cols.push_back(kRed+1);      
    files.push_back("qc_ladder_9.json");  cols.push_back(kRed-9);      
    files.push_back("qc_ladder_10.json"); cols.push_back(kBlue-4);    
    files.push_back("qc_ladder_11.json"); cols.push_back(kMagenta+1); 
    files.push_back("qc_ladder_12.json"); cols.push_back(kYellow+3);   
    files.push_back("qc_ladder_13.json"); cols.push_back(kGreen-2);       
    files.push_back("qc_ladder_14.json"); cols.push_back(kViolet+1);   
    files.push_back("qc_ladder_15.json"); cols.push_back(kCyan+2);    
    files.push_back("qc_ladder_16.json"); cols.push_back(kOrange-3);  
    files.push_back("qc_ladder_17.json"); cols.push_back(kOrange+5);  

    files.push_back("qc_ladder_26.json"); cols.push_back(kRed+1);     
    files.push_back("qc_ladder_27.json"); cols.push_back(kRed-9);      
    files.push_back("qc_ladder_28.json"); cols.push_back(kBlue-4);       
    files.push_back("qc_ladder_29.json"); cols.push_back(kMagenta+1);  
    files.push_back("qc_ladder_30.json"); cols.push_back(kYellow+3);  
    files.push_back("qc_ladder_31.json"); cols.push_back(kGreen-2);   
    files.push_back("qc_ladder_32.json"); cols.push_back(kViolet+1);  
    files.push_back("qc_ladder_33.json"); cols.push_back(kCyan+2);    
    files.push_back("qc_ladder_34.json"); cols.push_back(kOrange-3);   
    files.push_back("qc_ladder_35.json"); cols.push_back(kOrange+5);   
  };
...
// ----------------------------------------------------------------------
TGraphErrors* makeGraph(vector<pair<double, double> > result, int mcolor, int mstyle, double msize, bool absVal=false) {
  ...
  if (absVal) {
    for (unsigned int i = 1; i < result.size(); ++i) {
      result[i].first = TMath::Abs(result[i].first);
    }
  }
  ...
  for (unsigned int i = 1; i < result.size(); ++i) {
    for (unsigned int ix = 0; ix < x.size(); ++ix) {
      cout << " i/ix = " <<  i << "/" << ix
           << " comparing result[i].first = " << result[i].first << " with x[ix] = " << x[ix]
           << endl;
...
TGraphErrors* ivTest(string filename= "qc_ladder_0.json", int color = 1) {
  ...
  TGraphErrors *grG  = makeGraph(result, color, 20, 1., true);
  grG->Draw("l");
  return grG; 
}
...
void ivTests(int layer = 1) {
  ...
  // -- split drawing into US and DS for better overview
  for (int is = 0; is < 2; ++is) {
    c0.Clear();
    h1->Draw();
    for (unsigned int i = is*hists.size()/2; i < is*hists.size()/2 + hists.size()/2; ++i) {
      cout << "drawing histogram " << i << endl;
      hists[i]->Draw("pl");
    }
    
    TLegend *tll = newLegend(0.75, 0.25, 0.87, 0.45);
    if (0 == layer) {
      tll->SetTextSize(0.020);
      tll->SetHeader((is<1? "Layer 0 US": "Layer 0 DS"));
    } else if (1 == layer) {
      tll->SetTextSize(0.020);
      tll->SetHeader((is<1? "Layer 1 US": "Layer 1 DS"));
    }    
    //    for (unsigned int i = 0; i < files.size(); ++i) {
    for (unsigned int i = is*hists.size()/2; i < is*hists.size()/2 + hists.size()/2; ++i) {
    ...
    if (1 == layer) {
      c0.SaveAs((is<1? "iv-layer1-US.pdf": "iv-layer1-DS.pdf"));
    }
    if (0 == layer) {
      c0.SaveAs((is<1? "iv-layer0-US.pdf": "iv-layer0-DS.pdf"));
    }
  }
}
