...
G4VPhysicalVolume* Mu3eExperimentConstruction::Construct() {
    ...
    if(auto config = cfg.ptree.get_optional<std::string>("detector.pixels.customGeometry")) {
        mu3e::log::info(
            "[ExperimentConstruction::Construct] use pixels.customGeometry: %s\n",
            config->c_str()
        );
    ...
    if(phase > 0 && outerTrackerConstruction != nullptr) {
        ...
        auto tileSipmSD = new Mu3eTileSipmSD(
            "mu3e/TileSipmSD",
            tile,
            digicfg.isWriteTiles()
        );
        ...
        recurlStationConstruction = new Mu3eRecurlStationConstruction(
            largeSensor, tile,
            cfg,digicfg,
            logicalExperiment, outerTrackerConstruction->getLength()
        );
...
void Mu3eExperimentConstruction::initTransforms(
    const G4VPhysicalVolume* physicalVolume
) {
