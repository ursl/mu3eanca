...
void Mu3eRunAction::BeginOfRunAction(const G4Run* run) {
    ...
    if(!file) {
        ...
        if(boost::filesystem::exists(directory) && !boost::filesystem::is_directory(directory)) {
            mu3e::log::fatal("[RunAction] path '%s' is not a directory\n", boost::filesystem::absolute(directory).c_str());
            exit(1);
        }
        if(!boost::filesystem::exists(directory)) {
            boost::filesystem::create_directories(directory);
        }
        std::string fileName = fmt::format("{}/mu3e_run_{:06}.root", directory, runId);
        file = std::make_unique<mu3e::RootFile>(fileName, "RECREATE");
        mu3e::log::info("[RunAction] output = '%s'\n", fileName.c_str());
    }
    if(!file->IsOpen()) {
        mu3e::log::fatal("FATAL: Failed to create file: %s\n", file->GetName());
        exit(1);
    }
