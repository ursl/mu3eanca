...
void Mu3eFibreSmbSD::writeStat() {
  ...
  TH1F *hFibreSmbDose2 = new TH1F("hFibreSmbDose2", "Fibre Smb Dose", 200, 0, 200);
  hFibreSmbDose2->GetXaxis()->SetTitle("fibre Smb");
  hFibreSmbDose2->GetYaxis()->SetTitle("dose/time [Gy]");
  for(unsigned int i = 0; i < 200; ++i) {
    hFibreSmbDose2->SetBinContent(i+1, fDose[i]);
  }
