...
void FbDet::makeClusters() {
    ...
    for(auto iti = hits.begin(), itj = iti + 1, end = hits.end(); itj != end; ++itj) {
        Hit* hi = *iti;
        Hit* hj = *itj;
        // if (cntFb[hi->id.value()] == 1) {
        //   std::cout << "FbDet id.value() = " << hi->id.value()
        //             << " x = " << hi->x
        //             << std::endl;
        // }
        ...
        if(hi->id.mppcId.value() == hj->id.mppcId.value() &&
          ...
          // if (cntMppc[hi->id.mppcId.value()] == 1) {
          //   std::cout << "FbDet id.mppcId.value() = " << hi->id.mppcId.value()
          //             << " hi->id.mppc.v.x = " << hi->mppc.v.x
          //             << std::endl;
          // }
...
void FbDet::readFibres(Fibre::map_t& fibres, TTree* tree) {
  ...
  if (pDC->getDB()) {
    ...
    std::cout << "fibrealignment_ cal = " << cal << std::endl;
    
    calFibreAlignment *cpa = dynamic_cast<calFibreAlignment*>(cal);
    uint32_t i(99999);
    std::cout << "HALLO FbDet::readSensors(...) cpa->getName() = " << cpa->getName() << std::endl;
    int cnt(0); 
    unsigned int id;
...
void FbDet::readMppcs(Mppc::map_t& mppcs, TTree* tree) {
  ...
  if (pDC->getDB()) {
    ...
    std::cout << "mppcalignment_ cal = " << cal << std::endl;
    
    calMppcAlignment *cpa = dynamic_cast<calMppcAlignment*>(cal);
    uint32_t i(99999);
    std::cout << "HALLO FbDet::readMppcs(...) cpa->getName() = " << cpa->getName() << std::endl;
    int cnt(0); 
    unsigned int id;
    double3 v, dcol;
    while (cpa->getNextID(i)) {
    ...
    std::cout << "ROOT open alignment tree " << tree->GetName() << std::endl;
    tree->Scan("mppc:vx:vy:vz:colx:coly:colz:ncol");
    uint32_t id;
    double3 v, dcol;
    ...
    for(Long64_t n = tree->GetEntries(), entry = 0; entry < n; entry++) {
      ...
      mppc.v = make_float3(v);
      mppc.dcol = make_float3(dcol);
      std::cout << "FbDet::Mppcs root ID = " << id 
                << " v = " << v.x << "/" << v.y << "/" << v.z
                << " col = " << dcol.x << "/" << dcol.y << "/" << dcol.z
                << std::endl;
    }
