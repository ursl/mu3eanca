    ...
    , sensor(sensor), row(row), col(col), timestamp{timestamp_, Conf::inst.si.n_fine_bits}
{
    ...
    if(sensor.deform.surface.maxCoeff() > double(FLT_EPSILON) ||
...
void SiDet::init(const Sensor::map_t& sensors) {
    MU3E_INFO("init global variables\n");
    for(int i = 0; i < LAYER_N; i++) LAYER_R[i] =  LAYER_L[i] = 0;
    std::set<int> ladders[LAYER_N];
    ...
    for(int i = 0; i < LAYER_N; i++) {
        mu3e::printf("  layer %d : %d ladders, r = %.1f, l = %.1f\n",
            i, ladders[i].size(), LAYER_R[i], LAYER_L[i]
        );
    }
}
...
void SiDet::readSensors(Sensor::map_t& sensors, TTree* tree) {
    if (!mu3e::rec::conf.conddb.useCDB) {
      ...
      if (pDC->getDB()) {
        ...
        std::cout << "filling SiDet from CDB with pixelalignment_ cal = " << cal << std::endl;
