...
namespace mu3e {
  namespace sim {
    ...
    FibreSupport::FibreSupport(FibreDetector* detector_, const Mu3eDetectorCfg& cfg, const Mu3eDigiCfg& digi)
      ...
      cout << "XXXXXX length = " <<  length << " lengthSup = " << lengthSup << endl;
      ...
      G4VSolid* solidFibreSupportHolder = new G4Tubs(
                                                     "fibreSupportHolder",
                                                     rIn+rPlate, rOut,
                                                     (length - MppcLength - zOffset - 5*mm)/2.,
                                                     0, 2*M_PI);
      ...
      if (1) new G4PVPlacement(nullptr,
                               {0, 0, -lengtPlate/2. + length/2. + 8*mm},    
      ...
      if (1) new G4PVPlacement(nullptr,
                               {0, 0, + MppcLength/2. + zOffset/2. - 2*mm},
    ...
    void FibreSupport::PlaceMppcInSupport(G4LogicalVolume *vol, bool mirrored) {
      ...
      for(unsigned int i = 0; i < detector->nribbons; ++i) {
        ...
        if(i%2==1) {
          position.setZ(length/2. + MppcThicknessEpoxy/2. - MppcLength/2. - 14.17*mm);
        } else {
          position.setZ(length/2. + MppcThicknessEpoxy/2. - MppcLength/2. - 26.67*mm);
        }
        transform = G4Transform3D(rotM, position);
        G4ThreeVector pMax, pMin;
        vol->GetSolid()->BoundingLimits(pMin, pMax);

        if (fDBX) G4cout << "fibreMPPC " << i << " added at phi: " << phi
    ...
    void FibreSupport::PlaceSmbInSupport(G4LogicalVolume *vol, bool mirrored) {
      ...
      for(unsigned int i = 0; i < detector->nribbons; ++i) {
        ...
        positionPcb.setZ(position.z() - length/2.  - 4.3*mm);
        transform = G4Transform3D(rotM, positionPcb);
      ...
      int nSmb(-1);
      if (fDBX) cout << "Renaming components for mirrored = " << mirrored
                    << ", i.e. simpr = " << simpr
                    << endl;
      while (1) {
        ...
        if (string::npos != sname.find("Pcb_")) {
          ...
          if (fDBX) cout << "*ipv->GetName() = " << sname << " trsl = "
                        << (*ipv)->GetTranslation()
                        << " phiOrig = " << phiOrig
                        << " phi = " << phi
                        << " -> nSmb = " << nSmb
    ...
    G4AssemblyVolume* FibreSupport::makeSmb(int index) {
      ...
      if (fDBX) cout << "Placement fibreSMBAsic0 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      ...
      Ta.setX(tx); Ta.setY(ty); Ta.setZ(tz);
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBAsic1 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      if (DOASIC) solidFibreSMB->AddPlacedVolume(pa, Tr);
      ...
      Ta.setX(tx); Ta.setY(ty); Ta.setZ(tz);
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBAsic2 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      if (DOASIC) solidFibreSMB->AddPlacedVolume(pa, Tr);
      ...
      Ta.setX(tx); Ta.setY(ty); Ta.setZ(tz);
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBAsic3 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      if (DOASIC) solidFibreSMB->AddPlacedVolume(pa, Tr);
      ...
      Ta.setX(tx); Ta.setY(ty); Ta.setZ(tz);
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBChip2 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      ...
      Ta.setZ(tz);
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBChip3 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      solidFibreSMB->AddPlacedVolume(pa, Tr);
      ...
      Tr = G4Transform3D(rotm, Ta);
      if (fDBX) cout << "Placement fibreSMBChip1 T = (" << tx << "," << ty << "," << tz << ")" << endl;
      solidFibreSMB->AddPlacedVolume(pa, Tr);
      // -- rotate back
      rotm.rotateZ(+M_PI/4*CLHEP::rad);
