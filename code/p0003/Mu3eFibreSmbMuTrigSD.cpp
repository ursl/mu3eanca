    ...
    for (unsigned int ismb = 0; ismb < 12; ++ismb) {
      for (unsigned int iasic = 0; iasic < 4; ++iasic) {
        ...
        if (fFillDbxHist) {
          ...
          if (0 == iasic) {
            fSmbMuTrigEdep.insert(make_pair(sns, new TH1F(shs.c_str(), Form("MuTrig hits, Edep for %s %s", sz.c_str(), sns.c_str()),
                                                          100, 0., 1.0)));
          }
          ...
          if (0 == iasic) {
            fSmbMuTrigLZ.insert(make_pair(sns, new TH1F(shs.c_str(), Form("MuTrig hits, local z for %s %s", sz.c_str(), sns.c_str()),
                                                        nbins, -1., +1.)));
          }
          ...
          if (0 == iasic) {
            fSmbMuTrigLXY.insert(make_pair(sns, new TH2F(shs.c_str(), Form("MuTrig hits, local xy for %s %s", sz.c_str(), sns.c_str()),
                                                         100, -5., +5., 100, -5., +5.)));
          }
          ...
          if (0 == iasic) {
            fSmbMuTrigGXY.insert(make_pair(sns, new TH2F(shs.c_str(), Form("MuTrig hits, global xy for %s %s", sz.c_str(), sns.c_str()),
                                                         200, -100., +100., 200, -100., +100.)));
          }
        }
      }
