...
compareHist::compareHist(const std::string &infile1, const std::string &infile2) {
  ...
  // -- loop over all TDirectories in fInFile1 and search for ttype%d
  fInFile1->cd();
  ...
  for (int i = 0; i < list->GetEntries(); ++i) {
    ...
    if (sname.find("ttype") != std::string::npos) {
      fTrackTypes.push_back(sname);
      cout << "compareHist::compareHist() adding track type " << sname << endl;
    }
  }
  
  // -- now check that all these dirs are also in fInFile2
  fInFile2->cd();
  list = gDirectory->GetListOfKeys();
  for (auto &name : fTrackTypes) {
    bool found = false;
    for (int i = 0; i < list->GetEntries(); ++i) {
      TKey *key = (TKey*)list->At(i);
      TObject *obj = key->ReadObj();
    ...
    if (!found) {
      cout << "compareHist::compareHist() WARNING: directory " << name 
      << " not found in " << fInFileName2 << endl;
      fTrackTypes.erase(find(fTrackTypes.begin(), fTrackTypes.end(), name));
      cout << "compareHist::compareHist() WARNING: removing track type " << name << endl;
    }
  }
...
// ----------------------------------------------------------------------
TH1 *compareHist::getHist(TFile *f, const std::string &name, const std::string &dir) {
  if (!f) return nullptr;
